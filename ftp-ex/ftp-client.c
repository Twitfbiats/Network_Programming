...
int main() {
    ...
    ctrl_addr.sin_addr.s_addr = inet_addr("192.168.1.118");
...

    {
        ...
        struct sockaddr_in data_addr;
        data_addr.sin_family = AF_INET;
        data_addr.sin_addr.s_addr = inet_addr("192.168.1.118");
        data_addr.sin_port = htons(i1);
    ...
    // Download file
    {
        ...
        // Gui lenh RETR
        send(ctrl_socket, "RETR test1.mp4\r\n", 16, 0);
        len = recv(ctrl_socket, buf, sizeof(buf), 0);
        buf[len] = 0;
        puts(buf);

        // Nhan du lieu
        FILE *f = fopen("download1.mp4", "wb");
        while (1)
        {
            len = recv(data_socket, buf, sizeof(buf), 0);
            if (len <= 0)
